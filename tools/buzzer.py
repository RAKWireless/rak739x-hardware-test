...
buzzer_trigger_pin = 13
# Set Set default frequency value (Hz)
buzzer_frequency = 440
# Set default duration value (ms)
buzzer_duration = 500

parser = OptionParser('python3 buzzer.py '+ '-g <port> -f <frequency> -d <duration>')
# Defines the options
parser.add_option("-g", "--port", type="int", dest="port", help="GPIO as BCM_GPIO where the buzzer is connected to (defaults to 13, range is 0 to 27 )")
parser.add_option("-f", "--frequency", type="int", dest="frequency", help="Frequency in hertz of the tone to generate (defaults to 440 Hz(A4), range is 10 to 5000 Hz)")
parser.add_option("-d", "--duration", type="int", dest="duration", help="Duration in ms to play the tone (defaults to 1000 ms, range is 1 to 10000 ms)")
...
while isinstance(options.port, int):
    if options.port in range(0, 28):
        buzzer_trigger_pin = options.port
        break
    else:
...
count = int((buzzer_duration / 1000) / base_time)

with gpiod.request_lines(
    "/dev/gpiochip0",
    consumer="BUZZER",
    config={
        buzzer_trigger_pin: gpiod.LineSettings(
            direction=gpiod.line.Direction.OUTPUT, output_value=gpiod.line.Value.ACTIVE
        )
    },
) as request:
    while count:
        request.set_value(buzzer_trigger_pin, gpiod.line.Value.ACTIVE)
        time.sleep(delay_on)
        request.set_value(buzzer_trigger_pin, gpiod.line.Value.INACTIVE)
